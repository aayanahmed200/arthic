...
the concept should stick the first time, not the fifth time, at 1am, before the exam. arthic is an all-in-one exam-prep platform for Pakistan's toughest entrance tests: MDCAT, ECAT, NUST NET, FAST, CSS, PMS, and LUMS LNAT. Not just a chatbot — an AI mentor, organized past papers, a personalized study plan, and an exam calendar, all in one place.
...
Most exam prep for these seven exams still means the same thing: a stack of guides, a crowded academy, and an instructor who doesn't have time to explain a concept twice. arthic is a small, student-run team building a platform that meets students where they actually are — an AI mentor that keeps asking follow-up questions until a concept actually clicks instead of just marking an answer wrong, past papers organized by topic instead of a dumped PDF pile, a study plan personalized to an exam date, and an exam calendar so no deadline sneaks up. The mentor is built to notice when a student is pattern-matching instead of understanding, and to slow down exactly there instead of moving on to the next chapter.
...
A light "paper" background (`#fcfbf8`) and near-black ink (`#15140f`) rather than true black-on-white — closer to a printed page than a screen default — with a real light/dark theme toggle (persisted in `localStorage`, defaulting to the visitor's OS preference) that inverts the same two tokens rather than swapping in a separate dark stylesheet. One accent, emerald (`#0e8f63` decorative / `#0a6e4c` for text in light mode, brightened to `#17a674` / `#34c48f` in dark to hold contrast), used sparingly for links, active states, and exam tags. Two self-hosted faces — Bricolage Grotesque (display, variable weight) for everything from the hero to body copy, IBM Plex Mono reserved for labels, tags, and UI chrome. Motion is `cubic-bezier(0.16, 1, 0.3, 1)` almost everywhere — scroll reveals, hovers, smooth-scroll easing — one curve, used consistently.

The homepage hero also runs a static preview of the mentor dashboard next to the pitch — chrome bar, nav tabs, a sample identify/explain exchange, and a topic-mastery + streak panel — clearly labeled a preview (not a live product) in both its own chrome and a caption underneath, the same honesty register as the features-preview cards below it.
...
arthic is English-only for now. An earlier build shipped a full English/Urdu toggle (native script, RTL layout, a `[data-i18n]`-driven dictionary) — it's been pulled back out rather than left shipping half-working. The nav's globe button (`src/lib/lang-stub.ts`) is an honest stand-in: it opens a small "more languages — coming soon" tooltip instead of switching anything, no `localStorage`, no dictionary, no `dir="rtl"`. If full Urdu support ships later, this is the file (and this section) that gets replaced.
...
- [Lenis](https://github.com/darkroomengineering/lenis) for smooth scrolling; everything else (reveals, nav state, the section folio label, the theme toggle, the language-stub tooltip) is plain `IntersectionObserver`, `localStorage`, and `requestAnimationFrame` — no animation or i18n library
...
│   │   └── lib/        nav, common page bootstrap, reveal system, smooth scroll, theme toggle, language stub, waitlist form
...
│   ├── index.html      home — hero (+ mentor-dashboard preview), features preview, why arthic, board strip, hiring, closing
...
│   ├── careers.html    open roles, each linking to apply.html?role=…
│   ├── apply.html      one shared, detailed application form, relabeled per-role
