...
- **`frontend/`** — the marketing site. Vite + TypeScript, hand-written CSS (no UI framework, no Tailwind), a small generative-art module for the Orbital game page, and a scroll-reveal system that fails safe — content is real markup first, animation is a progressive enhancement on top of it, never a gate in front of it. Home, About, Games, Careers, and Journal are each real routes/pages, not anchors into one long scroll. Fetches the journal from the backend when one is deployed, falls back to static content baked into the page when it isn't. See `frontend/README.md`.
- **`backend/`** — the API behind the journal and the "get updates by email" form. Express + SQLite (via Node's built-in `node:sqlite`, so nothing native to compile), plus a small admin panel for writing journal entries without touching code. See `backend/README.md`.
...
The frontend talks to the backend at `http://localhost:3001` automatically in dev. In production it only does so if you set `VITE_API_BASE` at build time — see `frontend/README.md`. Without a backend deployed at all, the site still works: the journal shows its three static entries and the subscribe form says so honestly instead of pretending to succeed.
...
- Node.js, Express, `node:sqlite` for the backend
...
```
arthic/
├── frontend/          marketing site (Vite + TypeScript)
│   ├── src/
...
├── backend/            API (Express + node:sqlite)
│   └── src/
│       ├── routes/     journal + subscribers
│       └── public/admin/   the journal admin panel
├── LICENSE
...
- Set a real Discord invite (`VITE_DISCORD_URL` at build time, or edit `frontend/src/lib/config.ts` — search is easiest: the placeholder is `discord.gg/arthic`)
- Set `ADMIN_PASSWORD` on the backend host — never commit `.env`
- Point `VITE_API_BASE` at the deployed backend URL once one exists
...
See [DEPLOYMENT.md](DEPLOYMENT.md) for the actual Vercel + Render deploy steps and DNS records.
