/**
 * Static fallback for journal content — the same three seed entries the
 * backend ships with (backend/src/seed.js). journal.ts prefers the live
 * API when a backend is reachable, and falls back to this so the journal
 * is never a dead end, even with no backend deployed at all.
 */
...
It's slower now. It asks more of you in the first ten minutes and less of you after that, which is the opposite of how most of these curves are supposed to go. We think that's the right trade. We're still finding out.`,
  },
  {
    ...
    excerpt: "full price doesn't mean the same thing to everyone. here's how we're thinking about it.",
